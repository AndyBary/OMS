...
function read(){	
	...
	$qres = "SELECT COUNT(*) FROM questions WHERE theme_id_name='$theme_id' and archiv='0'";
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	//запрос для вопросов
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	$query = "SELECT quest_id, quest, quest_dt, quest_user, status FROM questions, themes WHERE theme_id_name = '$theme_id' and theme_id_name = theme_id and archiv='0' LIMIT $art,$kol";
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	parse_str($parts['query'], $queryurl);
	$kab = $queryurl['login'];
	echo "<button type='button' id='arch' class='btn btn-secondary btn-sm' onclick=";echo "window.location=";echo"'archiv.php?login=$kab'>Просмотреть архивные вопросы</button>";	
	echo "<br><br>	
	<form method='POST' action=''>
		<div class='input-group mb-3'>
			<input type='text' class='form-control' name='searchRow' placeholder='Поиск по вопросам' aria-label='Поиск по вопросам' aria-describedby='button-addon2'>
			<div class='input-group-append'>
				<input class='btn btn-secondary' type='submit' id='button-addon2' name='search' value='Найти'></inpit>										
			</div>
		</div>
	</form>";
	...
	if(mysqli_num_rows($result) < 1){
		echo "<h4>В данной теме еще нет вопросов</h4>";
	}
	else {
				...
				if(mysqli_num_rows($answer_result) == 0)
				{
					...
					<br><div class='btn btn-group'>
						<form method='POST'>
							...
							if ($theme_id == $admin_theme_row[0]){
							echo "<a type='button' href='#scroll' class='btn btn-outline-dark' name='idrowans' onclick='anShow($id);'>Ответить</a>";
							}
						echo "</form>					
					</div>
					</div>
				...
				else
				{
					...
					<br><div class='btn btn-group'>
						<form method='POST'>
							<input type='hidden' style='margin: 20px;' name='idrow' id='idrowinput$id' value='$id'></input>";
							if ($theme_id == $admin_theme_row[0]){
							echo "<textarea type='text' align='left' class='form-control' name='ansArea' cols='200' rows='2'></textarea>
							<br>
		...
		{	//вывод для обычного пользователя						
			while ($row=mysqli_fetch_row($result)) {
				...
				{					
					...
					<br><div class='btn btn-group'>
						<form method='POST'>";														
							echo "<input type='hidden' style='margin: 20px;' name='idrow' id='idrowinput$id' value='$id'></input>";
							if ($role_row[0] == 'moder'){
							echo "<input type='submit' class='btn btn-outline-danger' name='del' value='Удалить'></input>";
							}
						echo "</form>					
					</div>
					</div>
				...
				else //если есть ответ
				{					
					
					$id = $row[0];
					echo "<div class='divbg'>
					<h5 style='margin: 20px;'>Вопрос №$row[0]</h5>
					<i style='margin: 20px;'>От <u>$quest_user_row[0]</u> <b>($row[3])</b></i>
					<p><i style='margin: 10px;'>Задан $row[2]</i></p>
					<h6 style='margin: 10px;'> $row[1]</h6>
					<p><i style='margin: 10px;'>Ответ от <u>$answer_row_user[0]</u> <b>($answer_row[1])</b></i>
					<i style='margin: 10px;'>Получен $answer_row[2]</i></p>
					<h6 style='margin: 10px;'>$answer_row[0]</h6>
					</br>
					<br><div class='btn btn-group'>
						<form method='POST'>
							<input type='hidden' style='margin: 20px;' name='idrow' id='idrowinput$id' value='$id'></input>";
							if ($role_row[0] == 'user' and $row[3]==$ab){
							echo "<textarea type='text' class='form-control' name='ansArea' cols='200' rows='2' placeholder='Введите вопрос для уточнения'></textarea>
							<br>
							<input type='submit' class='btn btn-outline-success' name='ansChat' value='Отправить'></a>";
							}
							if ($role_row[0] == 'moder'){
							echo "<div class='btn btn-group'>
								...
								<input type='submit' class='btn btn-outline-primary' name='archiv' value='Перенести в архив'></input>
							</div>";
							}
						echo "</form>					
					</div>
...
//поле добавления новой темы (администратор)	
	...
	if($check_role_row[0] == "moder"){
		echo "<label>Добавление темы</label>
				<input name='themeId' type='hidden' value='$total'>
				<textarea name='newTheme' class='form-control' value='123'> </textarea>
				<br>
				<input name='createTheme' type='submit' value='Добавить' class='btn btn-success'>
				</br>
				</br>
				<button type='button' class='btn btn-secondary' data-toggle='modal' data-target='#swap' autocomplete='off'>Сменить тему администратора</button>";
	}	
		echo "</form>";
}
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="logForm"  method='POST'>
                            ...
                            <br />
                            <input type="text" class="form-control" id="loginus" name="loginus" value="<?php echo $fn;?>" required>
                            <br />
